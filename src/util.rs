use crate::personal_table::PersonalTable;
use crate::pk6::Pk6;
...
const PERSONAL_TABLE_RAW: &[u8] = include_bytes!("personal_ao");
pub const GENDER_SYMBOLS: [char; 3] = ['♂', '♀', '-'];

pub const EXP_TABLE: [[u32; 6]; 100] = [
...
lazy_static! {
    ...
    pub static ref MOVE_DATA: Vec<Vec<u8>> = {
        let mut data = Vec::with_capacity(622);
        for i in 0..622 {
            let mut buf = vec![];
            let mut file = File::open(format!("move/move_{:0>3}.bin", i)).unwrap();
...
pub fn get_string(string_bytes: Vec<u8>) -> String {
    string_bytes
        .chunks(2)
        .map(|chunk| {
            char::from_u32(u32::from(u16::from_le_bytes(chunk.try_into().unwrap()))).unwrap()
        })
        .collect::<Vec<char>>()
        .iter()
        .filter_map(|c| if *c == '\u{0}' { None } else { Some(c) })
        .collect::<String>()
}

pub fn reorder_teams(order: u8, style: u8, teams: &mut Vec<Vec<Pk6>>, trainers: &mut Vec<String>) {
    if (order & 0xA0) == 0xA0 && style == 4 {
        for p in 0..6 {
            let t1 = teams[0][p].clone();
            teams[0][p] = teams[2][p].clone();
            teams[2][p] = t1;
        }

        let t1 = trainers[0].clone();
        trainers[0] = trainers[2].clone();
        trainers[2] = t1;
    } else if (order & 0xA0) == 0xA0 {
        for p in 0..6 {
            let t1 = teams[0][p].clone();
            teams[0][p] = teams[1][p].clone();
            teams[1][p] = t1;
        }

        let t1 = trainers[0].clone();
        trainers[0] = trainers[1].clone();
        trainers[1] = t1;
    }
}
